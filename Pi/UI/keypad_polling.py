...
class keypad(object):
    ...
    def is_no_press(self):
        return self.GPIO.input(self.vert[0]) == self.GPIO.LOW and self.GPIO.input(self.vert[1]) == self.GPIO.LOW and self.GPIO.input(self.vert[2]) == self.GPIO.LOW and self.GPIO.input(self.vert[3]) == self.GPIO.LOW
    ...
    def poll_for_num(self):
        ...
        while True:
            ...
            for y in range(len(self.vert)): #check input at each vert
                if isLow and self.GPIO.input(self.vert[y]) == self.GPIO.HIGH: # closed switch detected
                    if first_press: # button press should stop reading of prompt
                        self.speaker.stop()
                        print "\n\n\nSTOP\n\n\n"
                        first_press = False
        ...
        while not self.chr_queue.empty():
            ...
            for y in range(len(self.vert)): #check input at each vert
                if isLow and self.GPIO.input(self.vert[y]) == self.GPIO.HIGH: # closed switch detected
                    if self.num_map[y][x] != 9 and self.num_map[y][x] != 11:
                        if self.en_snd:
                            self.chr_queue.append(self.num_map[y][x], time.time())
                            self.voiceSema.release()
                    while GPIO.input(self.vert[y]) == GPIO.HIGH:
                        pass
                    return self.num_map[y][x] #update num_pressed
                    ...
                    if self.num_map[y][x] == 11:
                        ...
                        if self.en_snd:
                            if self.num_map[y][x] == 10:
                                self.chr_queue.append(0, time.time())
                                self.voiceSema.release()
                            else:
                                self.chr_queue.append(self.num_map[y][x] + 1, time.time())
                                self.voiceSema.release()
    ...
    def string_gen(self, num_pressed):
        ...
        if num_pressed == self.VOID_PRESS:
            if self.curr_chr != '':
                ...
                if self.en_snd:
                    self.chr_queue.append(str(self.curr_chr), time.time())
                    self.voiceSema.release()
            self.curr_chr = ''
            self.prev_num = -3
            self.num_count = 0
        ...
        #     self.prev_num = -3 #update keypress history
            ...
            self.out_str = ""
            if self.en_snd:
                self.chr_queue.append("clear", time.time())
                self.voiceSema.release()
            self.curr_chr = '' #clear curr_chr
            self.num_count = 0 #reset count on key map
            self.prev_num = -3 #update keypress history
            ...
            if num_pressed >= 12: #hold down keys
                if self.curr_chr != '':
                    self.out_str += self.curr_chr
                    if self.en_snd:
                        self.chr_queue.append(self.curr_chr, time.time())
                        self.voiceSema.release()
                self.out_str += self.key_map[num_pressed][0]
                if self.en_snd:
                    self.chr_queue.append(self.key_map[num_pressed][0], time.time())
                    self.voiceSema.release()
                ...
                if self.prev_num != -3: #first press condition
                    self.out_str += self.curr_chr
                    if self.en_snd:
                        self.chr_queue.append(self.curr_chr, time.time()) ##self.speaker.speak(self.key_map[self.prev_num][self.num_count])
                        self.voiceSema.release()
                self.num_count = 0 #reset count on keymap
